...
namespace SteerLib
{
    ...
    private:
        ...
        static Util::Vector support_function(const std::vector<Util::Vector>& shapeA, const std::vector<Util::Vector>& shapeB, const Util::Vector& direction);
        
        static Util::Vector central_origin_of_shape(const std::vector<Util::Vector>& shape);
        
        static Util::Vector Minkowski_Helper(const std::vector<Util::Vector>& shape, const Util::Vector& direction);
        
        static float dot(const Util::Vector& vectorA, const Util::Vector& vectorB);
        
        static void epa(const std::vector<Util::Vector>& _shapeA, const std::vector<Util::Vector>& _shapeB, std::vector<Util::Vector>& simplex, float& penetration_depth, Util::Vector& penetration_vector);
        
        static void findNearestEdge(std::vector<Util::Vector> simplex, float& distance, Util::Vector& normal, int& index);
